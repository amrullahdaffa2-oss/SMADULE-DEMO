...
export function Navbar() {
  ...
  const { role, notifikasi, markNotifRead, markAllRead, setSidebarOpen, sidebarOpen } = useAppStore()
  ...
  return (
    <header className="sticky top-0 z-20 h-16 flex items-center px-6 bg-white/80 backdrop-blur-sm border-b border-slate-200 gap-4">
      ...
      <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-xl text-sm text-slate-400 cursor-pointer hover:bg-slate-200 transition-colors w-48">
        <Search size={14} />
      ...
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-white font-semibold text-xs shrink-0">
        {user.avatar}
      </div>
    </header>
  )
